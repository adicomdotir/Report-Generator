...
export class HealingComponent implements OnInit {
	...
	healingcalc() {
		...
		let age = this.age;
		let weeks = this.injuryweeks;
		let docs = this.mediclevel;
		let sublevel = weeks;

		let local3 = (table[(age - 17)][docs]) * 10;
		let local4 = Number.parseFloat(((sublevel - 0.99) / local3).toFixed(2));
		let local5 = Number.parseFloat((sublevel / local3).toFixed(2));
		let local6 = [0, 0];
		let local7 = [0, 0];
		...
		this.update_b = local4;
		this.week_b = local6[0];
		this.day_b = local6[1];
		this.update = local5;
		this.week = local7[0];
		this.day = local7[1];
